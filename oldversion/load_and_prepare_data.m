...
%         if ~isfield(df, required_fields{i})
%             error(['缺少必要的字段: ', required_fields{i}]);
%         end
%     end
%     
%     %% 基本特征计算
...
%     df.PDI = 100 * movmean(df.positive_DM, 14) ./ movmean(df.TR, 14);
%     df.NDI = 100 * movmean(df.negative_DM, 14) ./ movmean(df.TR, 14);
%     df.DX = 100 * abs(df.PDI - df.NDI) ./ (df.PDI + df.NDI);
%     df.ADX = movmean(df.DX, 14);
%     
%     %% 成交量特征
...
%         features{end+1} = ['BB_lower_' num2str(p)];movmean(df.close, [p-1 0]) - 2 * std_p;
...
%     X = [];
%     parfor ii = 100:length(df.close)
%         dftemp=structfun(@(x) x(ii-99:ii), df, 'UniformOutput', false);
%         Xtemp = load_and_prepare_data_real(dftemp);
%         X = [X; Xtemp(end,:)];
%     end
    ...
    % 首先删除包含整行 NaN 的行
    nan_rows = all(isnan(X), 2); % 找到整行都是 NaN 的行
    X(nan_rows, :) = [];         % 删除这些行
