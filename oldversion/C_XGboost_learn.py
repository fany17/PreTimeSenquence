...
def load_and_prepare_data(file_path):
    ...
    # Parabolic SAR
    df['PSAR'] = ta.psar(high=df['high'], low=df['low'], close=df['close'])
...
def train_xgboost_model(model, X_train, y_train, X_val, y_val, model_save_path="xgboost_model.json"):
    ...
    print(f"XGBoost 模型已保存到 {model_save_path}")
...
def main():
    ...
    # 训练 XGBoost 模型
    train_xgboost_model(xgb_model, X_train, y_train, X_val, y_val, model_save_path)

    # 加载模型并预测
    xgb_model.load_model(model_save_path)
    predict_and_evaluate(xgb_model, X_test, y_test)
